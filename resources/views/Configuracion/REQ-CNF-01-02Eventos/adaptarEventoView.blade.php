...
@section('content')
    ...
    @foreach ($t1 as $t1) 
    <form action="{{url('configuracion/adaptarevento/'. $id)}}" method ="post" enctype="multipart/form-data">
    @csrf
        <label for="nombre"> Nombre Evento </label>
        <input type="text" name="nombre" value="{{$t1->nombre}}"  id="nombre">
        @if ($errors->has('nombre'))
            <span class="error text-danger">{{$errors->first('nombre')}}</span>
        @endif
        <br>
        <label for="lugar"> Lugar del Evento </label>
        <input type="text" name="lugar" value="{{$t1->lugar}}" id="lugar">
        @if ($errors->has('lugar'))
            <span class="error text-danger">{{$errors->first('lugar')}}</span>
        @endif
        <br>
        <label for="fecha_inicio"> Fecha de Inicio </label>
        <input type="date" name="fecha_inicio" value="{{$t1->fecha_inicio}}" id="fecha_inicio">
        @if ($errors->has('fecha_inicio'))
            <span class="error text-danger">{{$errors->first('fecha_inicio')}}</span>
        @endif
        <br>
        <label for="fecha_fin"> Fecha de Fin </label>
        <input type="date" name="fecha_fin" value="{{$t1->fecha_fin}}" id="fecha_fin">
